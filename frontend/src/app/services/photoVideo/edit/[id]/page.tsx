...
const UpdatePhotoVideoListing = () => {
  ...
  useEffect(() => {
    const fetchPhotoVideoData = async () => {
      try {
        const { data: photoVideo, error } = await supabase
        ...
        if (photoVideo) {
          // Basic Information
          setBusinessName(photoVideo.business_name);
          setExperience(photoVideo.years_experience);
          setTravelRange(photoVideo.travel_range?.toString() || "");
          setIsWillingToTravel(photoVideo.travel_anywhere);
          setDescription(photoVideo.description);
          setIsRemoteBusiness(photoVideo.is_remote_business);
          setServiceType(photoVideo.service_type);
          setWebsiteUrl(photoVideo.website_url || "");
          setInstagramUrl(photoVideo.instagram_url || "");
          setAvailability({ deposit: photoVideo.deposit?.toString() || "" });

          // Location
          setLocation({
            enteredLocation: `${photoVideo.address || ""}, ${
              photoVideo.city
            }, ${photoVideo.state}, ${photoVideo.country}`.trim(),
            address: photoVideo.address || "",
            city: photoVideo.city || "",
            state: photoVideo.state || "",
            country: photoVideo.country || "",
            placeId: photoVideo.place_id || "",
          });

          // Handle specialties
          if (
            photoVideo.photo_video_specialties &&
            Array.isArray(photoVideo.photo_video_specialties)
          ) {
            const photoStyles: string[] = [];
            const videoStyles: string[] = [];
            const customPhotoStylesList: string[] = [];
            const customVideoStylesList: string[] = [];

            photoVideo.photo_video_specialties.forEach((specialty: any) => {
              if (specialty.style_type === "photography") {
                if (commonPhotoStyles.includes(specialty.specialty)) {
                  photoStyles.push(specialty.specialty);
                } else {
                  customPhotoStylesList.push(specialty.specialty);
                }
              } else if (specialty.style_type === "videography") {
                if (commonVideoStyles.includes(specialty.specialty)) {
                  videoStyles.push(specialty.specialty);
                } else {
                  customVideoStylesList.push(specialty.specialty);
                }
              }
            });

            setSpecialties([...photoStyles, ...videoStyles]);
            setCustomPhotoStyles(customPhotoStylesList);
            setCustomVideoStyles(customVideoStylesList);
          }

          // Handle media files
          if (
            photoVideo.photo_video_media &&
            Array.isArray(photoVideo.photo_video_media)
          ) {
            const existingIds: string[] = [];
            const mediaFilesList = await Promise.all(
              photoVideo.photo_video_media.map(async (media: any) => {
                const {
                  data: { publicUrl },
                } = supabase.storage
                  .from("photo-video-media")
          ...
          if (
            ...
            Array.isArray(photoVideo.photo_video_services)
          ) {
            const services: { [key: string]: Service } = {};
            const customServicesList: Service[] = [];

            photoVideo.photo_video_services.forEach((service: any) => {
              const serviceObj = {
                name: service.name,
              ...
              if (
                commonServices.some((cs) => cs.name === service.name) ||
                commonVideoServices.some((cvs) => cvs.name === service.name)
              ) {
        ...
        console.error("Error fetching photo/video data:", error);
  ...
  const handleMediaUpload = async (
    ...
    try {
      ...
      const mediaPromises = newFiles.map(async (file, index) => {
        ...
        const filePath = `photoVideo/${businessId}/${index}-${Date.now()}.${fileExt}`;
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (photoVideoError) {
        throw new Error(
          `Failed to update Photography & Videography listing: ${photoVideoError.message}`
        );
      }
      if (!photoVideo) {
        throw new Error(
          "Photography & Videography listing updated but no data returned"
        );
      }
      ...
      const allServices = [
        ...
        ...(serviceType === "videography" || serviceType === "both"
          ? Object.values(selectedServices)
              .filter((service) =>
                commonVideoServices.some((cs) => cs.name === service.name)
              )
              .map((service) => ({
                business_id: id,
                name: service.name,
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to update Photography & Videography listing. Please try again."
      );
    } finally {
  ...
  return (
    <ProtectedRoute>
      <ListingOwnerProtectedRoute tableName="photo_video_listing">
...
export default UpdatePhotoVideoListing;
